...
import sys
sys.path.append(r'/hpcvol1/siennaw/lib/stompy//') # SW Added --> for debugging
...
if restart is None:
    print('Coming up with initial conditions ...')
else:
    print('RESTART SELECTED: Run will be started using %s' % restart)
    Restart = True 
...
response = input('Now created all the folders for the run. Do you want to stop run in order to copy over wind + temperature files? (y/n)')
if response == 'y' :
    print('Stopping run!')
    sys.exit() 
...
mdu['physics', 'Temperature']  = 5 # Include temperature (0: no, 1: only transport, 3: excess model of D3D, 5: composite (ocean) model)']
mdu['physics', 'Soiltempthick']  = 0.1  
...
# WIND + TEMPERATURE 
# =============================================================================
windx = list(run_base_dir.glob('*.amu')) 
if len(windx) > 0 : 
#     response = input('Use %s for wind_x? (y/n)' % windx[0])
#     if response == 'y':
    print('Using %s for wind_x' % windx[0])
    lines = ['QUANTITY=windx',
             'FILENAME=%s' % windx[0].name , 
             'FILETYPE=4',
             'METHOD=1',
             'OPERAND=O\n']
    with open(str(old_bc_fn) ,'at') as fp:
...
windy = list(run_base_dir.glob('*.amv')) 
if len(windy) > 0 : 
# response = input('Use %s for wind_y? (y/n)' % windy[0])  
# if response == 'y':
    print('Using %s for wind_y' % windy[0])
    lines = ['QUANTITY=windy',
             'FILENAME=%s' % windy[0].name,
             'FILETYPE=4',
             'METHOD=1',
             'OPERAND=O\n']
    with open(str(old_bc_fn) ,'at') as fp:
...
tem = list(run_base_dir.glob('*.tem')) 
if len(tem) > 0 : 
    # response = input('Use %s for humidity_airtemperature_cloudiness? (y/n)' % tem[0])  
    # if response == 'y':
    print('Using %s for humidity_airtemperature_cloudiness' % tem[0])
    lines = ['QUANTITY=humidity_airtemperature_cloudiness',
             'FILENAME=%s' % tem[0].name,
             'FILETYPE=6',
             'METHOD=3',
             'OPERAND=O\n']
    with open(str(old_bc_fn) ,'at') as fp:
...
if Restart:
    filename = Path(restart).name
    restart_destination = run_base_dir / filename
    if not restart_destination.exists():
        print('Copying over intial condition %s ...' % filename)
        copy_string  = 'cp %s %s' % (restart, restart_destination)
        os.system(copy_string)   # Copy initial condition over to run folder 
    mdu['restart','RestartFile'] = filename
else: 
